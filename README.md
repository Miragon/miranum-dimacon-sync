...
Kunden-Paginierung). Nach Dimacon schreibt der Mitarbeiter-Abgleich nur mit
dem Schritt „Mitarbeiter in Dimacon anlegen" — aus einem eigenen
Dimacon-Topf, also bis zu `CONCURRENCY_DIMACON` offene Dimacon-Requests
zusätzlich. Dazu die Sammelabrufe, die an keinem Limit hängen. Die Phasen nach dem Zusammenführen (Projekt-Upserts, Archivierung)
...
# … inklusive Übernahme Lexware → Dimacon (Belege der letzten 14 Tage)
curl -X POST http://localhost:3020/api/integrations/dimacon-lexoffice/run \
  -H "Content-Type: application/json" \
  -d '{ "dryRun": true, "steps": { "importFromLexware": true } }'
...
- `dimacon-clockin/` — Orchestrator (fail-soft pro Projekt), Employee-Matching
  der Tagesplanung (Personalnummer), Kunden-Upsert, Projekt-Upsert mit
  Mitarbeiter-Diff (attach/detach), Archivierung; `employee-sync/` darin ist
  der bidirektionale Stammdaten-Abgleich (Matching nur über die
  Personalnummer, aktive vor archivierten, Dimacon gewinnt) als erster
  Schritt; `employee-sync/creation-policy.ts` entscheidet den Relevanzfilter
  beider Anlage-Richtungen
- `dimacon-lexoffice/` — Lexware-Kontakt find-or-create + Kundennummern-Alignment.
  ...
  Kunden-Kontakte (keine Lieferanten, keine archivierten). Opt-in-Gegenrichtung:
  `voucher-candidates.ts` (Belegliste) → `import-policy.ts` (Relevanzfilter) →
  `importer.ts` (Anlage in Dimacon). Die Felder des neuen Dimacon-Kunden
